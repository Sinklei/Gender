...
#names(GII)[4]<- "HDI Platzierung"
#names(GII)[5]<- "Land"
#names(GII)[6] <-"GII Wert"
#names(GII)[7]<- "GII Platzierung"
#names(GII)[8]<- "Müttersterblichkeitsrate (Tode pro 100.000 Lebendgeburten)"
#names(GII)[9]<- "Jugendgeburtenrate (Geburten pro 1.000 Frauen im Alter von 15-19 Jahren)"
#names(GII)[10]<- "Anteil von Frauensitze im Parlament (in %)"
#names(GII)[11]<- "Frauen über 25 Jahren mit mindestens Sekundarbschluss (in %)"
#names(GII)[12]<- "Männer über 25 Jahren mit mindestens Sekundarbschluss (in %)"
#names(GII)[13]<- "Erwerbsquote der weiblichen Bevölkerung ab 15 Jahren (in %)"
#names(GII)[14]<- "Erwerbsquote der männlichen Bevölkerung ab 15 Jahren (in %)"
```
...
3. fancy Tabelle: beste Länder, schlechteste Länder + DL. Wie das aussehen soll: Links neben den Ländern sieht man die Länderflagge, und den Platz (#1). In den Daten steht direkt % oder so dahinter. Die auffälligsten Werte durch Farbe markierien (zB Chaf Müttersterb., Chad Frauen Sekundarabschluss, Papua Neu  Guinea Frauen im Parlament -> rot markieren/ grün markieren: Schweden Frauen Parlament, Frauen mind. Sekundarabschluss Deutschland  ) 
...
# in jamovi die englischen Ländernamen ins deutsche übersetzt: data/toworkwith/ GII_world
GII_world <- read_csv("data/to_work_with/GII_world.csv")
...
mytext <- paste(
    ...
    "GII-Platzierung:", GII_world$GII_Rank, "<br/>", 
...
reactable(GII_fil2,
          columns = list(
            Land = colDef(cell = function(value) {
      ...
      tagList(
        ...
        value
            ...
            Müttersterblichkeitsrate = colDef(name= "Müttersterblichkeitsrate (Tode pro 100.000 Lebendeburten)",format = colFormat(digits = 1) ),
            Jugendgeburtenrate = colDef(name= "Geburtenrate bei Jugendlichen (pro 1.000 Frauen im Alter von 15-19 Jahren", format = colFormat(digits = 1) ),
            Frauensitze= colDef(name = "Anteil von Frauensitze im Parlament", format = colFormat(suffix = "%", digits = 1)),
            Frauen_Sekundarbschluss = colDef(name=  "Frauen über 25 Jahren mit mindestens Sekundarbschluss", format = colFormat(suffix = "%", digits = 1)), 
            Männer_Sekundarbschluss= colDef(name="Männer über 25 Jahren mit mindestens Sekundarbschluss", format = colFormat(suffix = "%", digits = 1)),
            Erwerbsquote_weiblich= colDef(name= "Erwerbsquote der weiblichen Bevölkerung ab 15 Jahren", format = colFormat(suffix = "%", digits = 1)),
            Erwerbsquote_männlich = colDef(name = "Erwerbsquote der männlichen Bevölkerung ab 15 Jahren", format = colFormat(suffix = "%", digits = 1))))


#Flaggen erstellen und in Tabelle einbetten 
#Flaggen von: https://www.kaggle.com/zhongtr0n/country-flag-urls Gifs in Png umwandeln
```
