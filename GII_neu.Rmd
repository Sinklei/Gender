...
options(digits=3)
GII <- read.csv("data/to_work_with/GII.csv")

rownames(GII)

arrange(GII, GII$GII_Rank)
```

## GI Ländernamen waren auf englisch, vorher in jamovi ins deutsche übersetzt
```{r setup, include=FALSE}
GII_bearb <- read.csv("data/to_work_with/GII_bearb.csv")
```


## Landkarte mit dem GII_bearbeitet Datensatz
```{r setup, include=FALSE}
...
mypalette <- colorBin( palette="YlOrBr", domain=GII_bearb$GII_Value , na.color="transparent", bins=mybins)

mytext <- paste(
    "Land:", GII_bearb$name_long ,"<br/>", 
    "GII-Platzierung:", GII_bearb$GII_Rank, "<br/>", 
    "GII-Wert:", GII_bearb$GII_Value, 
    sep="") %>%
...
GII_Landkarte <- leaflet(GII_bearb) %>% 
  addTiles()  %>% 
