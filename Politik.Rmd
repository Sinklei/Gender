...
seats_world %>% 
  ...
  geom_bar(position = position_stack(reverse = F)) +
  scale_x_continuous(breaks = c(1997, 2020)) +
  scale_fill_manual(values = c("#A8D8FF", "#F5504E")) +
  theme(axis.line.x = element_line(arrow = arrow()),
        legend.title = element_blank()) +
  geom_hline(yintercept = 50,
             size = 1) +
  labs(y = NULL, x = "Zeitraum von 1997 bis 2020")
...
#####
text <- list(
        werte = as.character(seats_world$percent),
        jahre = as.character(seats_world$Jahr)
)
