...
bundestag <- bundestag_raw[c(1,2,3,7,6,4,5)] %>%          #variablen in eine sinnvolle reihenfolge bringen
...
bundestag %>% 
  ...
  geom_area(aes(y = mitglieder, x = as.numeric(bundestag)), fill = "black", alpha = 0.2) +
  geom_area(aes(y = maenner_anzahl, x = as.numeric(bundestag)), fill = "red", alpha = 0.2) +
  geom_area(aes(y = frauen_anzahl, x = as.numeric(bundestag)), fill = "yellow", alpha = 0.2)



             

# mitglieder umbenennen
# einfacher line plot aber wo markante werte als größere punkte rauskommen??
# wo noch text hinzufügen können?
...
ds <- data.frame(partei, sitze, sitze_w, sitze_m)
...
ds <- ds %>% 
  mutate(prop = ds$sitze/sum(ds$sitze)) %>% 
  mutate(p_end = cumsum(ds$prop)) %>% 
  mutate(p_start = c(0, head(ds$p_end, -1))) %>% 
  mutate(mid_angle = 0.5*(p_start + p_end))


ds$prop <- ds$sitze/sum(ds$sitze) 
ds$p_end <- cumsum(ds$prop)
ds$p_start <- c(0, head(ds$p_end ,-1))
ds$mid_angle <- 0.5*(p_start + p_end)
...
# haöbkreis probieren
pie %>% 
  mutate_at(c("start_angle", "end_angle"), rescale, to=pi*c(-0.5, 0.5), from = 0:1) %>% 
  ggplot() +
  aes(x0 = 0, y0 = 0, r0 = 0.5, r = 1,
      start = start_angle,
      end = end_angle,
