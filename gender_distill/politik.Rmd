...
Wir konnten bereits sehen, dass uns geschlechtliche Ungleichheiten im Alltag erscheinen.

Wir möchten uns die geschlechtliche Ungleichheit mal an einem konkreten Beispiel anschauen. Dazu schauen wir in die Politik!

Unsere letzte Bundestagswahl liegt noch nicht lange zurück, weshalb es sich lohnt, mal einen Blick auf die Geschlechterverteilung in der Politik zu werfen.

Bevor wir uns jedoch unserem Deutschen Bundestag widmen, lasst es erst einmal einen Blick auf die gesamte Welt werfen.
...
seats_world %>% 
  ...
  labs(
    ...
    subtitle = "Der Frauenanteil lag 2020 nur bei 24 %!",
...
es wird sehr eindeutig, dass in der politik annähernd keine gleichstellung vorhanden ist. 2020 lag der mittelwert bei 24 Prozent! erfreulicherweise sehen wir dennoch einen klaren anstieg in den zahlen. 1997 lag der wert bei gerade einmal 10 prozent. es kommen zwar nur langsam mehr frauen in parlamente, aber dennoch stetig! wir sind gespannt, wie diese entwicklung weitergeht. was meint ihr? wann werden wir eine komplette gleichstellung haben?

<br>
<br>

Wir haben nun gesehen, wie die Zahlen auf der gesamten Welt sind. Jetzt stellt sich die Frage, wie sieht es bei uns in Europa aus?
Was auf den wie ein Spaghetti-Teller oder das Londoner U-Bahn Netz aussieht, sind tatsächlich die Verläufe der europäischen Länder. Schaut am besten Mal selbst, wie sich Verteilungen in den Parlamenten der europäischen Länder verändert haben. Entweder ihr schaut auch mit einem Doppelklick ein Land an oder ihr wählt mehrere aus. Wir haben euch auch den Verlauf der gesamten Welt, den ihr oben bereits gesehen habt, dargestellt.

am besten schaut ihr einfach mal selbst! dazu könnt ihr ganz einfach die länder auswählen, die euch am meisten interessieren. mit einem doppelklick schaut ihr euch das land einzeln an.
...
spaghetti <- europe_final %>% 
 ...
 labs(
    title = "Prozentualer Frauenanteilen in Parlamenten europäischer Länder",
    x = NULL,
    y = NULL
  ) +
  theme(
...
in Europa ist der Frauenanteil deutlich höher im vergleich zur gesamten welt. hier liegt er bei ca 30%. das ist doch super (zumindest besser)!
unterschiede zwischen den ländern

<br>
<br>

Nachdem wir nun geschaut haben, wie Deutschland im Vergleich zu den anderen europäischen Ländern und der Welt abschneidet (hier der Hinweis: schaut es euch noch an ;)), wollen wir uns jetzt spezifischer den Deutschen Bundestag anschauen.

Dazu haben wir hier zuerst die Entwicklung des Deutschen Bundestag vom ersten bis zum aktuellen Bundestag. Hier werden nicht die Prozente anzeigt wie bisher, sondern die konkrete Anzahl an Abgeordneten. 
...
lasst uns also mal unseren aktuellen Bundestag anschauen. gerade weil gleichberechtitung zwischen den geschlechtern sehr aktuell und brisant ist, lohnt es sich anzuschauen, wie die verteilung zwischen männern und frauen in der deutschen politik ist.

dazu schauen wir uns erst mal detaillierter die sitzeverteilung zwischen den parteien an. vielleicht hat das ja auch etwas mit dem frauenanteil zu tun?! was denkst du dazu?
...
Nun wollen wir uns mal die Geschlechterverteilung innerhalb der Parteien anschauen.
...
seats_bundestag %>% 
  ...
  labs(
    ...
    subtitle = "Frauenanteil ist in den Farben der Parteien dargestellt",
    caption = "Quelle: Seite des Bundestags [3]",
    x = "Anzahl der Sitze",
    y = NULL) +
  theme_minimal_hgrid(
...
WIr können sehen dass sososososoososos
